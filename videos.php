...
//$query_videos=mysqli_query($con,"SELECT * FROM top_videos");
...
<title>
talent of punjab videos punjabi talent
</title>
...
?>
    ...
    <div class="row">
        <div class="col-md-2">
			...
			<a href="?category=mix"><li>
			<span style="<?php if($cat=='null'||$cat=='mix'||$cat=='') { echo 'background:#3300b2;color:white;border-radius:5px;padding:2px;'; } ?>">Mix</span>
			</li>
			</a>
			...
			<li><span style="<?php if($cat=='innovation') { echo 'background:#3300b2;color:white;border-radius:5px;padding:2px;'; } ?>">
			Innovation
			</span>
			</li>
			</a>
...
if($cat=="mix")
{
$rows=mysql_num_rows(mysql_query("select * from top_videos where verified='ok'"));
$_SESSION['store_cat_v']="mix";
}
else if($cat=="song")
{
$rows=mysql_num_rows(mysql_query("select * from top_videos where category='song' and verified='ok'"));
$_SESSION['store_cat_v']="song";
}
elseif($cat=="funny")
{
$rows=mysql_num_rows(mysql_query("select * from top_videos where category='funny' and verified='ok'"));
$_SESSION['store_cat_v']="funny";
}
elseif($cat=="innovation")
{
$rows=mysql_num_rows(mysql_query("select * from top_videos where category='innovation' and verified='ok'"));
$_SESSION['store_cat_v']="innovation";
}
elseif($cat=="talent")
{
$rows=mysql_num_rows(mysql_query("select * from top_videos where category='talent' and verified='ok'"));
$_SESSION['store_cat_v']="talent";
}

elseif($cat=="other")
{
$rows=mysql_num_rows(mysql_query("select * from top_videos where category='other' and verified='ok'"));
$_SESSION['store_cat_v']="other";
} 
else
{
	$rows=mysql_num_rows(mysql_query("select * from top_videos where verified='ok'"));
...
if($cat=="mix")
{
$query=mysql_query("select * from top_videos where verified='ok' LIMIT $start, $limit");
}
elseif($cat=="song")
{
	$query=mysql_query("select * from top_videos where category='song' and verified='ok' LIMIT $start, $limit");
}
elseif($cat=="funny")
{
	$query=mysql_query("select * from top_videos where category='funny' and verified='ok' LIMIT $start, $limit");
}
elseif($cat=="innovation")
{
	$query=mysql_query("select * from top_videos where category='innovation' and verified='ok' LIMIT $start, $limit");
}
elseif($cat=="talent")
{
	$query=mysql_query("select * from top_videos where category='talent' and verified='ok' LIMIT $start, $limit");
}
elseif($cat=="other")
{
	$query=mysql_query("select * from top_videos where category='other' and verified='ok' LIMIT $start, $limit");
}
else
{
	$query=mysql_query("select * from top_videos where verified='ok' LIMIT $start, $limit");
...
<tr>
			<td>
			<a href="video_detail.php?v_id=<?php echo $query2[v_id];?>">
			<img src="images/video_thumb4.png" width="130" height="120" style="position:absolute;z-index:1;">
            <img src="video_thumbs/<?php echo $query2[v_id].".png"; ?>" width="130" height="120">
			
			</a>
			</td>
			
			<td><a href="video_detail.php?v_id=<?php echo $query2[v_id];?>" ><div style="padding-left:7px;color:#0000b3"><b style="color:#3300b2;text-decoration:underline"><?php echo $query2[title]; ?></b></a><br>
			<?php echo "size: ".$query2[size]." kb" ?><br>

			cat.-<a href="videos.php?category=<?php echo $query2[category] ?>"><?php echo $query2[category] ?></a>
...
<div style="margin:10px;font-size:15px;color:#3300B2;font-weight:bold;" align="center">Current page:<?php
	...
	<td>
<?php
$cat_store=$_SESSION["store_cat_v"];
if($id>1)
{
	?>
