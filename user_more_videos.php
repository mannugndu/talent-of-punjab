...
<title>
user more videos punjabi talent talent of punjab
</title>
...
$rows=mysql_num_rows(mysql_query("select * from top_videos where user_id='$current_user' and verified='ok'"));
...
if(isset($_GET['id']))
{
	...
	$query=mysql_query("select * from top_videos where user_id='$current_user' and verified='ok' LIMIT $start, $limit");
...
<tr>
			...
			<source src="videos/<?php echo $query2[name]; ?>"></video></a>
			</td>
			...
			<?php echo $query2[size]." kb" ?><br>
			<?php echo "cat.-".$query2[category] ?>
			</div>
			</td>
			</tr>
<?php
