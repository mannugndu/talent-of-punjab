...
$title=$title;
mysqli_query($con,"insert into top_photos(title,category,description,user_id,time)values('$title','$category','$description','$user_id','$upload_time')");
...
document.getElementById("file1").style.visibility="hidden";
	...
	ajax.open("POST", "photo_upload_parser.php");
	ajax.send(formdata);
    }
}
function progressHandler(event){
	document.getElementById("browse").innerHTML="Please wait";
...
<li class="completed">
 ...
 <li id="finish">  <span class="bubble"></span>Finish</li> 
			...
			<tr>
			<td><div style="color:red" align="center">Extensions alloweded:- .jpg,.jpeg</div>
			</td>
			</tr>
			<tr>
			<td>&nbsp;
            ...
            <input type="file" name="file1" id="file1" onchange="file_check()">
...
<?php
	...
	$prev_name=mysqli_query($con,"select name from top_photos where p_id=(select MAX(p_id) from top_photos)");
